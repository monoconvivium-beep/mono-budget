/**
 * MonoConvivium â€” che funzioni anche senza rete.
 *
 * I movimenti stanno giÃ  tutti nel telefono: l'unica cosa che manca, senza
 * campo, sono i file dell'app. Questo li tiene da parte.
 *
 * âš ï¸ QUANDO SI PUBBLICA UNA VERSIONE NUOVA, CAMBIARE `VERSIONE` QUI SOTTO.
 * Senza, i telefoni che hanno giÃ  l'app continuano a mostrare quella vecchia e
 * sembra che le correzioni non siano state fatte.
 */
const VERSIONE = "mono-money-v25";

/**
 * Tutto si calcola dalla RADICE dove il guardiano Ã¨ registrato, mai da "/".
 * CosÃ¬ l'app funziona sia sul suo dominio, sia dentro una sottocartella
...
self.addEventListener("install", (evento) => {
  evento.waitUntil(
    (async () => {
      ...
      // Uno per uno: se un file non c'Ã¨, non deve far fallire tutta
...
self.addEventListener("fetch", (evento) => {
  ...
  /**
   * Aprire una pagina: prima la rete, la copia solo se non c'Ã¨ campo.
   * ðŸ”‘ Non il contrario. Con la copia per prima, dopo una pubblicazione il
   * telefono continuerebbe a caricare la pagina vecchia â€” che punta a pezzi di
   * programma che non esistono piÃ¹ â€” e l'app si aprirebbe rotta.
   */
  if (richiesta.mode === "navigate") {
    evento.respondWith(
      (async () => {
        ...
        try {
          /**
           * â±ï¸ TRE SECONDI E NON DI PIÃ™ (aggiunto il 15/8/2026).
           * ðŸ”´ Prima si aspettava la rete senza limite. Con una tacca di campo
           * â€” il parcheggio del supermercato, l'ascensore â€” una richiesta puÃ²
           * restare appesa mezzo minuto prima di arrendersi: in quel mezzo
           * minuto l'app Ã¨ una schermata bianca, e chi la apre pensa che sia
           * rotta e la chiude. La copia da parte Ã¨ giÃ  nel telefono e si apre
           * subito: passati tre secondi si usa quella. Nessuno aspetta
           * mezzo minuto per segnare un caffÃ¨.
           */
          ...
          /**
           * ðŸ”´ SI SALVA SOLO SE LA RISPOSTA Ãˆ BUONA.
           * Prima si metteva da parte QUALUNQUE cosa tornasse â€” anche la
           ...
           * âš ï¸ Le pagine interne (`/spesa`) su GitHub tornano **404 con dentro
          ...
          return (
            salvata ??
            new Response("Sei senza rete e l'app non Ã¨ ancora stata salvata.", {
              status: 503,
   ...
   * contenuto cambia il nome, quindi una copia salvata non Ã¨ mai vecchia.
   */
  evento.respondWith(
    (async () => {
      const cache = await caches.open(VERSIONE);
